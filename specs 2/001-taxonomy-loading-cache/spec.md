...
- **Taxonomy Cache Entry**: A cache slot holding a parsed Taxonomy Structure keyed by its entry point identifier and version, with metadata about when it was loaded and its filesystem source path. In-memory only in v1; on-disk persistence (requiring serialization) is deferred to a future version.
