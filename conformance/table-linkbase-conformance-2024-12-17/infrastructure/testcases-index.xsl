...
exclude-result-prefixes="fo">
	...
	<xsl:template match="testcase">
		...
		<tr>
			<td>
				<xsl:if test="document($uri)/conf:testcase/@minimal='false'">*</xsl:if>
			</td>
			<td align="right">
				<xsl:variable name="variations" select="count(document($uri)/conf:testcase/conf:variation)"/>
				<xsl:value-of select="$variations"/>
			</td>
			<td>
				<xsl:element name="a">
					<xsl:attribute name="href"><xsl:value-of select="@uri"/></xsl:attribute>
					<xsl:value-of select="document($uri)/conf:testcase/@name"/>
				</xsl:element>
			</td>
			<td>
				<xsl:element name="a">
					<xsl:variable name="owner" select="document($uri)/conf:testcase/@owner"/>
					<xsl:attribute name="href">mailto:<xsl:value-of select="$owner"/></xsl:attribute>
					<xsl:value-of select="$owner"/>
				</xsl:element>
			</td>
			<td>
				<xsl:variable name="description" select="document($uri)/conf:testcase/@description"/>
