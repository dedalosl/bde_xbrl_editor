...
class TestDiscoveryBasic:
    def test_minimal_entry_point(self, tmp_path):
        ...
        schemas, linkbases, _, _ = discover_dts(xsd, LoaderSettings())
    ...
    def test_discovers_imported_schema(self, tmp_path):
        ...
        schemas, _, _, _ = discover_dts(entry, LoaderSettings())
    ...
    def test_discovers_linkbase(self, tmp_path):
        ...
        _, linkbases, _, _ = discover_dts(entry, LoaderSettings())
...
"""
        ...
        schemas, _, _, _ = discover_dts(entry, LoaderSettings(allow_network=False))
...
class TestNetworkBlock:
    def test_network_blocked_skips_remote_url_gracefully(self, tmp_path):
        ...
        schemas, _, skipped, _ = discover_dts(entry, LoaderSettings(allow_network=False))
    ...
    def test_local_catalog_resolves_remote_url(self, tmp_path):
        ...
        entry = write_xsd(tmp_path, "entry.xsd", XSD_WITH_REMOTE)
        schemas, _, skipped, _ = discover_dts(entry, settings)
...
"""
        ...
        schemas, _, _, _ = discover_dts(tmp_path / "a.xsd", LoaderSettings())
        schema_names = {p.name for p in schemas}
